...
integer(kind=iint), parameter, public :: nr_of_proc = 24
...
!    ``none``
!******
    ! nr_of_species = 4, spuck = 2
    real(kind=rdouble), dimension(0:3, 1:2), intent(out) :: occupation
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model '003_Ru_local_interactio' was written by    |"
        print *, "|                                                            |"
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                ...
                select case(layer)
                case (ru_slab)
                    call replace_species((/i, j, k, ru_slab_p1/), null_species, ru)
                    call replace_species((/i, j, k, ru_slab_p2/), null_species, ru)
                end select
            end do
        end do
    end do
...
subroutine recalculate_rates_matrix()
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_0,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_0,(/ i, j, k, 1/),gr_ru_interaction_0((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_1,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_1,(/ i, j, k, 1/),gr_ru_interaction_1((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_10,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_10,(/ i, j, k, 1/),gr_ru_interaction_10((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_11,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_11,(/ i, j, k, 1/),gr_ru_interaction_11((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_12,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_12,(/ i, j, k, 1/),gr_ru_interaction_12((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_13,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_13,(/ i, j, k, 1/),gr_ru_interaction_13((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_14,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_14,(/ i, j, k, 1/),gr_ru_interaction_14((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_15,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_15,(/ i, j, k, 1/),gr_ru_interaction_15((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_16,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_16,(/ i, j, k, 1/),gr_ru_interaction_16((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_17,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_17,(/ i, j, k, 1/),gr_ru_interaction_17((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_18,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_18,(/ i, j, k, 1/),gr_ru_interaction_18((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_19,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_19,(/ i, j, k, 1/),gr_ru_interaction_19((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_2,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_2,(/ i, j, k, 1/),gr_ru_interaction_2((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_20,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_20,(/ i, j, k, 1/),gr_ru_interaction_20((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_21,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_21,(/ i, j, k, 1/),gr_ru_interaction_21((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_22,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_22,(/ i, j, k, 1/),gr_ru_interaction_22((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_23,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_23,(/ i, j, k, 1/),gr_ru_interaction_23((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_3,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_3,(/ i, j, k, 1/),gr_ru_interaction_3((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_4,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_4,(/ i, j, k, 1/),gr_ru_interaction_4((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_5,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_5,(/ i, j, k, 1/),gr_ru_interaction_5((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_6,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_6,(/ i, j, k, 1/),gr_ru_interaction_6((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_7,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_7,(/ i, j, k, 1/),gr_ru_interaction_7((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_8,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_8,(/ i, j, k, 1/),gr_ru_interaction_8((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
    ...
    do i=1, system_size(1)
        do j=1, system_size(2)
            do k=1, system_size(3)
                if(can_do(ru_interaction_9,(/ i, j, k, 1/))) then
                    call update_rates_matrix(ru_interaction_9,(/ i, j, k, 1/),gr_ru_interaction_9((/ i, j, k, 0/)))
                end if
            end do
        end do
    end do
...
!******
    ...
    select case(proc)
    case(ru_interaction_0)
        call run_proc_ru_interaction_0(cell)

    case(ru_interaction_1)
        call run_proc_ru_interaction_1(cell)

    case(ru_interaction_10)
        call run_proc_ru_interaction_10(cell)

    case(ru_interaction_11)
        call run_proc_ru_interaction_11(cell)

    case(ru_interaction_12)
        call run_proc_ru_interaction_12(cell)

    case(ru_interaction_13)
        call run_proc_ru_interaction_13(cell)

    case(ru_interaction_14)
        call run_proc_ru_interaction_14(cell)

    case(ru_interaction_15)
        call run_proc_ru_interaction_15(cell)

    case(ru_interaction_16)
        call run_proc_ru_interaction_16(cell)

    case(ru_interaction_17)
        call run_proc_ru_interaction_17(cell)

    case(ru_interaction_18)
        call run_proc_ru_interaction_18(cell)

    case(ru_interaction_19)
        call run_proc_ru_interaction_19(cell)

    case(ru_interaction_2)
        call run_proc_ru_interaction_2(cell)

    case(ru_interaction_20)
        call run_proc_ru_interaction_20(cell)

    case(ru_interaction_21)
        call run_proc_ru_interaction_21(cell)

    case(ru_interaction_22)
        call run_proc_ru_interaction_22(cell)

    case(ru_interaction_23)
        call run_proc_ru_interaction_23(cell)

    case(ru_interaction_3)
        call run_proc_ru_interaction_3(cell)

    case(ru_interaction_4)
        call run_proc_ru_interaction_4(cell)

    case(ru_interaction_5)
        call run_proc_ru_interaction_5(cell)

    case(ru_interaction_6)
        call run_proc_ru_interaction_6(cell)

    case(ru_interaction_7)
        call run_proc_ru_interaction_7(cell)

    case(ru_interaction_8)
        call run_proc_ru_interaction_8(cell)

    case(ru_interaction_9)
        call run_proc_ru_interaction_9(cell)
