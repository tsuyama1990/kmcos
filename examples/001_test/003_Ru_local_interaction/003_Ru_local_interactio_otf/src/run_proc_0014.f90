...
subroutine run_proc_ru_interaction_20(cell)
...
! Disable processes
    ...
    if(can_do(ru_interaction_0,cell + (/ 1, 2, 0, 1/))) then
        call del_proc(ru_interaction_0,cell + (/ 1, 2, 0, 1/))
    end if
    if(can_do(ru_interaction_1,cell + (/ 1, 1, 0, 1/))) then
        call del_proc(ru_interaction_1,cell + (/ 1, 1, 0, 1/))
    end if
    ...
    if(can_do(ru_interaction_18,cell + (/ 0, 1, 0, 1/))) then
        call del_proc(ru_interaction_18,cell + (/ 0, 1, 0, 1/))
    end if
    ...
    if(can_do(ru_interaction_19,cell + (/ 0, 1, -1, 1/))) then
        call del_proc(ru_interaction_19,cell + (/ 0, 1, -1, 1/))
    end if
    if(can_do(ru_interaction_2,cell + (/ 0, 2, 0, 1/))) then
        call del_proc(ru_interaction_2,cell + (/ 0, 2, 0, 1/))
    end if
    ...
    if(can_do(ru_interaction_21,cell + (/ 0, 0, -1, 1/))) then
        call del_proc(ru_interaction_21,cell + (/ 0, 0, -1, 1/))
    end if
    ...
    if(can_do(ru_interaction_22,cell + (/ -1, 0, 0, 1/))) then
        call del_proc(ru_interaction_22,cell + (/ -1, 0, 0, 1/))
    end if
    ...
    if(can_do(ru_interaction_23,cell + (/ -1, 0, -1, 1/))) then
        call del_proc(ru_interaction_23,cell + (/ -1, 0, -1, 1/))
    end if
    ...
    if(can_do(ru_interaction_4,cell + (/ -1, 1, 0, 1/))) then
        call del_proc(ru_interaction_4,cell + (/ -1, 1, 0, 1/))
    end if
    if(can_do(ru_interaction_5,cell + (/ -1, 0, 0, 1/))) then
        call del_proc(ru_interaction_5,cell + (/ -1, 0, 0, 1/))
    end if

! Update the lattice
    call replace_species(cell + (/0, 1, 0, ru_slab_p1/),empty,ru)
...
! Enable processes
    ...
    case(empty)
        call add_proc(ru_interaction_11, cell + (/ 0, 1, 0, 1/), gr_ru_interaction_11(cell + (/ 0, 1, 0, 0/)))
    end select

    select case(get_species(cell + (/-1, 0, 0, ru_slab_p2/)))
    case(empty)
        call add_proc(ru_interaction_7, cell + (/ 0, 1, 0, 1/), gr_ru_interaction_7(cell + (/ 0, 1, 0, 0/)))
    case(ru)
    ...
    case(empty)
        call add_proc(ru_interaction_4, cell + (/ 0, 1, 0, 1/), gr_ru_interaction_4(cell + (/ 0, 1, 0, 0/)))
    end select

    select case(get_species(cell + (/-1, 0, 0, ru_slab_p1/)))
    case(empty)
        call add_proc(ru_interaction_0, cell + (/ 0, 1, 0, 1/), gr_ru_interaction_0(cell + (/ 0, 1, 0, 0/)))
    end select

    select case(get_species(cell + (/-1, 1, 0, ru_slab_p1/)))
    case(empty)
        call add_proc(ru_interaction_1, cell + (/ 0, 1, 0, 1/), gr_ru_interaction_1(cell + (/ 0, 1, 0, 0/)))
    end select
    ...
    case(empty)
        call add_proc(ru_interaction_10, cell + (/ 0, 1, 0, 1/), gr_ru_interaction_10(cell + (/ 0, 1, 0, 0/)))
    end select
    ...
    case(empty)
        call add_proc(ru_interaction_5, cell + (/ 0, 1, 0, 1/), gr_ru_interaction_5(cell + (/ 0, 1, 0, 0/)))
    end select
    ...
    case(empty)
        call add_proc(ru_interaction_6, cell + (/ 0, 1, 0, 1/), gr_ru_interaction_6(cell + (/ 0, 1, 0, 0/)))
    end select

    select case(get_species(cell + (/0, 0, -1, ru_slab_p2/)))
    case(empty)
        call add_proc(ru_interaction_8, cell + (/ 0, 1, 0, 1/), gr_ru_interaction_8(cell + (/ 0, 1, 0, 0/)))
    end select
...
end subroutine run_proc_ru_interaction_20
