...
integer(kind=iint), parameter, public :: Fe = 0
integer(kind=iint), parameter, public :: Pt = 1
...
integer(kind=iint), parameter, public :: process_1 = 1
integer(kind=iint), parameter, public :: process_10 = 2
integer(kind=iint), parameter, public :: process_11 = 3
integer(kind=iint), parameter, public :: process_12 = 4
integer(kind=iint), parameter, public :: process_2 = 5
integer(kind=iint), parameter, public :: process_3 = 6
integer(kind=iint), parameter, public :: process_4 = 7
integer(kind=iint), parameter, public :: process_5 = 8
integer(kind=iint), parameter, public :: process_6 = 9
integer(kind=iint), parameter, public :: process_7 = 10
integer(kind=iint), parameter, public :: process_8 = 11
integer(kind=iint), parameter, public :: process_9 = 12
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model 'FePtBulk' was written by                   |"
        print *, "|                                                            |"
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                select case(layer)
                case(fept)
                    call touchup_fept_origin((/i, j, k, fept_origin/))
                end select
            end do
        end do
    end do
...
!******
    ...
    select case(proc)
    case(process_1)
        call take_Pt_fept_origin(lsite)
        call put_Fe_fept_origin(lsite)
        call take_Fe_fept_origin(lsite + (/0, 1, 1, 0/))
        call put_Pt_fept_origin(lsite + (/0, 1, 1, 0/))

    case(process_10)
        call take_Pt_fept_origin(lsite + (/0, 1, 1, 0/))
        call put_Fe_fept_origin(lsite + (/0, 1, 1, 0/))
        call take_Fe_fept_origin(lsite)
        call put_Pt_fept_origin(lsite)

    case(process_11)
        call take_Pt_fept_origin(lsite + (/1, 0, 1, 0/))
        call put_Fe_fept_origin(lsite + (/1, 0, 1, 0/))
        call take_Fe_fept_origin(lsite)
        call put_Pt_fept_origin(lsite)

    case(process_12)
        call take_Pt_fept_origin(lsite + (/1, 1, 0, 0/))
        call put_Fe_fept_origin(lsite + (/1, 1, 0, 0/))
        call take_Fe_fept_origin(lsite)
        call put_Pt_fept_origin(lsite)
    ...
    case(process_2)
        call take_Pt_fept_origin(lsite)
        call put_Fe_fept_origin(lsite)
        call take_Fe_fept_origin(lsite + (/1, 0, 1, 0/))
        call put_Pt_fept_origin(lsite + (/1, 0, 1, 0/))

    case(process_3)
        call take_Pt_fept_origin(lsite)
        call put_Fe_fept_origin(lsite)
        call take_Fe_fept_origin(lsite + (/1, 1, 0, 0/))
        call put_Pt_fept_origin(lsite + (/1, 1, 0, 0/))
...
subroutine put_Fe_fept_origin(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, empty, Fe)

    ! enable affected processes
    select case(get_species(site + (/0, -1, -1, 0/)))
    case(Pt)
        call add_proc(process_1, site + (/0, -1, -1, 0/))
    end select

    select case(get_species(site + (/0, 1, 1, 0/)))
    case(Pt)
        call add_proc(process_10, site)
    end select

    select case(get_species(site + (/1, 0, 1, 0/)))
    case(Pt)
        call add_proc(process_11, site)
    end select

    select case(get_species(site + (/1, 1, 0, 0/)))
    case(Pt)
        call add_proc(process_12, site)
    end select

    select case(get_species(site + (/-1, 0, -1, 0/)))
    case(Pt)
        call add_proc(process_2, site + (/-1, 0, -1, 0/))
    end select

    select case(get_species(site + (/-1, -1, 0, 0/)))
    case(Pt)
        call add_proc(process_3, site + (/-1, -1, 0, 0/))
    end select

    select case(get_species(site + (/0, -1, 1, 0/)))
    case(Pt)
        call add_proc(process_4, site + (/0, -1, 1, 0/))
    end select

    select case(get_species(site + (/1, 0, -1, 0/)))
    case(Pt)
        call add_proc(process_5, site)
    end select

    select case(get_species(site + (/-1, 1, 0, 0/)))
    case(Pt)
        call add_proc(process_6, site + (/-1, 1, 0, 0/))
    end select

    select case(get_species(site + (/0, 1, -1, 0/)))
    case(Pt)
        call add_proc(process_7, site)
    end select

    select case(get_species(site + (/-1, 0, 1, 0/)))
    case(Pt)
        call add_proc(process_8, site + (/-1, 0, 1, 0/))
    end select

    select case(get_species(site + (/1, -1, 0, 0/)))
    case(Pt)
        call add_proc(process_9, site)
    end select


end subroutine put_Fe_fept_origin

subroutine take_Fe_fept_origin(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, Fe, empty)

    ! disable affected processes
    if(avail_sites(process_1, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (-1), site(4) + (0)), 2).ne.0)then
        call del_proc(process_1, site + (/0, -1, -1, 0/))
    endif

    if(avail_sites(process_10, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_10, site)
    endif

    if(avail_sites(process_11, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_11, site)
    endif

    if(avail_sites(process_12, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_12, site)
    endif

    if(avail_sites(process_2, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (-1), site(4) + (0)), 2).ne.0)then
        call del_proc(process_2, site + (/-1, 0, -1, 0/))
    endif

    if(avail_sites(process_3, lattice2nr(site(1) + (-1), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(process_3, site + (/-1, -1, 0, 0/))
    endif

    if(avail_sites(process_4, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (1), site(4) + (0)), 2).ne.0)then
        call del_proc(process_4, site + (/0, -1, 1, 0/))
    endif

    if(avail_sites(process_5, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_5, site)
    endif

    if(avail_sites(process_6, lattice2nr(site(1) + (-1), site(2) + (1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(process_6, site + (/-1, 1, 0, 0/))
    endif

    if(avail_sites(process_7, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_7, site)
    endif

    if(avail_sites(process_8, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (1), site(4) + (0)), 2).ne.0)then
        call del_proc(process_8, site + (/-1, 0, 1, 0/))
    endif

    if(avail_sites(process_9, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_9, site)
    endif


end subroutine take_Fe_fept_origin

subroutine put_Pt_fept_origin(site)
    ...
    select case(get_species(site + (/0, 1, 1, 0/)))
    case(Fe)
        call add_proc(process_1, site)
    end select

    select case(get_species(site + (/0, -1, -1, 0/)))
    case(Fe)
        call add_proc(process_10, site + (/0, -1, -1, 0/))
    end select

    select case(get_species(site + (/-1, 0, -1, 0/)))
    case(Fe)
        call add_proc(process_11, site + (/-1, 0, -1, 0/))
    end select

    select case(get_species(site + (/-1, -1, 0, 0/)))
    case(Fe)
        call add_proc(process_12, site + (/-1, -1, 0, 0/))
    end select

    select case(get_species(site + (/1, 0, 1, 0/)))
    case(Fe)
        call add_proc(process_2, site)
    end select

    select case(get_species(site + (/1, 1, 0, 0/)))
    case(Fe)
        call add_proc(process_3, site)
    end select

    select case(get_species(site + (/0, 1, -1, 0/)))
    case(Fe)
        call add_proc(process_4, site)
    end select

    select case(get_species(site + (/-1, 0, 1, 0/)))
    case(Fe)
        call add_proc(process_5, site + (/-1, 0, 1, 0/))
    end select

    select case(get_species(site + (/1, -1, 0, 0/)))
    case(Fe)
        call add_proc(process_6, site)
    end select

    select case(get_species(site + (/0, -1, 1, 0/)))
    case(Fe)
        call add_proc(process_7, site + (/0, -1, 1, 0/))
    end select

    select case(get_species(site + (/1, 0, -1, 0/)))
    case(Fe)
        call add_proc(process_8, site)
    end select

    select case(get_species(site + (/-1, 1, 0, 0/)))
    case(Fe)
        call add_proc(process_9, site + (/-1, 1, 0, 0/))
    end select


end subroutine put_Pt_fept_origin

subroutine take_Pt_fept_origin(site)
    ...
    ! disable affected processes
    if(avail_sites(process_1, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_1, site)
    endif

    if(avail_sites(process_10, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (-1), site(4) + (0)), 2).ne.0)then
        call del_proc(process_10, site + (/0, -1, -1, 0/))
    endif

    if(avail_sites(process_11, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (-1), site(4) + (0)), 2).ne.0)then
        call del_proc(process_11, site + (/-1, 0, -1, 0/))
    endif

    if(avail_sites(process_12, lattice2nr(site(1) + (-1), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(process_12, site + (/-1, -1, 0, 0/))
    endif

    if(avail_sites(process_2, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_2, site)
    endif

    if(avail_sites(process_3, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_3, site)
    endif

    if(avail_sites(process_4, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_4, site)
    endif

    if(avail_sites(process_5, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (1), site(4) + (0)), 2).ne.0)then
        call del_proc(process_5, site + (/-1, 0, 1, 0/))
    endif

    if(avail_sites(process_6, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_6, site)
    endif

    if(avail_sites(process_7, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (1), site(4) + (0)), 2).ne.0)then
        call del_proc(process_7, site + (/0, -1, 1, 0/))
    endif

    if(avail_sites(process_8, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(process_8, site)
    endif

    if(avail_sites(process_9, lattice2nr(site(1) + (-1), site(2) + (1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(process_9, site + (/-1, 1, 0, 0/))
    endif


end subroutine take_Pt_fept_origin

subroutine touchup_fept_origin(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    if (can_do(process_1, site)) then
        call del_proc(process_1, site)
    endif
    if (can_do(process_10, site)) then
        call del_proc(process_10, site)
    endif
    if (can_do(process_11, site)) then
        call del_proc(process_11, site)
    endif
    if (can_do(process_12, site)) then
        call del_proc(process_12, site)
    endif
    if (can_do(process_2, site)) then
        call del_proc(process_2, site)
    endif
    if (can_do(process_3, site)) then
        call del_proc(process_3, site)
    endif
    if (can_do(process_4, site)) then
        call del_proc(process_4, site)
    endif
    if (can_do(process_5, site)) then
        call del_proc(process_5, site)
    endif
    if (can_do(process_6, site)) then
        call del_proc(process_6, site)
    endif
    if (can_do(process_7, site)) then
        call del_proc(process_7, site)
    endif
    if (can_do(process_8, site)) then
        call del_proc(process_8, site)
    endif
    if (can_do(process_9, site)) then
        call del_proc(process_9, site)
    endif
...
end subroutine touchup_fept_origin
