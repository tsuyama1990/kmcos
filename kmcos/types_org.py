...
#            pass
    ...
    def validate_model(self):
        ...
        for x in self.get_processes():
            print("process", self.get_processes)
        ...
        for process in self.get_processes():
            print(np.array([bs.coord.offset for bs in process.bystander_list]))
            for x in process.bystander_list:
                if len([y for y in process.bystander_list
                    ...
                    print(('Found more than one bystander for %s\n'
                                       % x.coord) +
                                      ('on process %s' % process.name))
                if len([y for y in process.condition_list if x.coord == y.coord]) > 0:
                    ...
                    # print('Process %s has both a condition and a bystander\n'
                    #                   'on %s!' % (process.name, x.coord))
                if len([y for y in process.action_list if x.coord == y.coord]) > 0:
                    ...
                    # print('Process %s has an action and a bystander\n on %s!' %
                    #                   (process.name, x.coord))
...
class LayerList(FixedObject, list):
    ...
    Attributes:
        - cell (np.array): Size of the unit-cell (3x3).
        - default_layer (str): Name of the default layer.
        - name (str): Name of the layer.
        - representation: Represents something related to the layer.
        - substrate_layer: Indicates the presence of a substrate layer.
    """
    ...
    def __init__(self, **kwargs):
        ...
        cell_value = kwargs.get('cell', None)
        
        if cell_value:
            if type(cell_value) is str:
                cell_array = np.array([float(i) for i in cell_value.split()])
            elif type(cell_value) is np.ndarray:
                if len(cell_value) == 9:
                    self.cell = cell_value.reshape(3, 3)
                elif len(cell_value) == 3:
                    self.cell = np.diag(cell_value)
                else:
                    raise UserWarning(f"{cell_value} not understood")
        else:
    ...
    def set_representation(self, images):
            """
            Set the representation based on the input images. If more than one representation is present,
            they should be sorted by their name.

            Parameters:
            -----------
            images : list, str, or ase.atoms.Atoms
                The input images. If a list, it should contain ASE Atoms or their representation. 
                If a string, it's a direct representation. If an ASE Atoms instance, its representation is used.

            Raises:
            -------
            UserWarning:
                If the data type of images is not understood.
            ...
            if isinstance(images, list):
                repr = '['
                for atoms in sorted(images, key=lambda atoms: get_ase_constructor(atoms)):
                    repr += f'{get_ase_constructor(atoms)}, '
                repr += ']'
                self.representation = repr
            elif isinstance(images, str):
                self.representation = images
            elif isinstance(images, ase.atoms.Atoms):
                self.representation = f'[{get_ase_constructor(images)}]'
            else:
                raise UserWarning(f"Data type {type(images)} of {images} not understood.")

    def __setattr__(self, key, value):
        """
        Overrides the attribute setting behavior for the class instance.
        If 'representation' attribute is being set, it processes and updates related attributes.

        Parameters:
        -----------
        key : str
            The attribute key being set.
        value : any
            The value being assigned to the attribute.

        Notes:
        ------
        If 'representation' is set with a truthy value, it evaluates the value to an ASE object.
        If 'representation' doesn't exist or is empty, it attempts to set the 'cell' attribute based on the value.
        It sets 'self.representation' to a string representing the ASE object's constructor.
    ...
    def generate_coord_set(self, size=[1, 1, 1], layer_name='default', site_name=None):
        """Generates a set of coordinates around the unit cell with the desired size.
        
        Args:
            size (list, optional): A list specifying the size in each dimension (default is [1, 1, 1]).
            layer_name (str, optional): Name of the layer to generate coordinates (default is 'default').
            site_name (str, optional): Name of the specific site in the layer (default is None).

        Returns:
            list: List of Coord objects representing the generated coordinates.

        Raises:
            UserWarning: If the provided layer name is not found in the project.
            UserWarning: If the provided site name is not found in the layer.
        """
        
        def generate_list_range(n):
            """
            Generates a list of integers within the range from -n (inclusive) to n (inclusive).

            Args:
                n (int): The half-range for generating the list (inclusive).

            Returns:
                list: A list of integers within the specified range.

            Examples:
                >>> generate_list_range(3)
                [-3, -2, -1, 0, 1, 2, 3]
                
                >>> generate_list_range(1)
                [-1, 0, 1]
            """
        ...
        if layers:
            ...
            raise UserWarning(f'No Layer named {layer_name} found.')

        if site_name is not None and not any([re.search(site_name, x) for x in ['_'.join(x.name.split('_')) for x in layer.sites]]):
            raise UserWarning(f'Layer {layer_name} has no site matching {site_name}. Please check spelling and try again.')

        if site_name is None:
            return [
                self.generate_coord(f"{site.name}.({i}, {j}, {k}).{layer_name}")
                for i in generate_list_range(size[0])
                for j in generate_list_range(size[1])
                for k in generate_list_range(size[2])
                for site in layer.sites
            ]
        else:
            # Confirm if the site_name matches with the registered one actually
            selected_site_names = [
                site.name
                for site in layer.sites
                if re.search(site_name, site.name)
            ]

            # Generate the sites around the site_name
            return [
                self.generate_coord(f"{site}.({i}, {j}, {k}).{layer_name}")
                for i in generate_list_range(size[0])
                for j in generate_list_range(size[1])
                for k in generate_list_range(size[2])
    ...
    def generate_coordset_offsets(self, size=[1, 1, 1], layer_name='default', site_name=None, offset=[0,0,0]):
        """Generates a set of coordinates around the unit cell with the desired size.
        ...
        def generate_list_range(n, offset_value):
            """
            Generates a list of integers within the range from -n (inclusive) to n (inclusive).
            ...
            Examples:
                ...
                [-1, 0, 1]
            """
            return list(range(-n+offset_value, n + 1 + offset_value))
        ...
        if site_name is None:
            return [
                self.generate_coord(f"{site.name}.({i}, {j}, {k}).{layer_name}")
                for i in generate_list_range(size[0], offset[0])
                for j in generate_list_range(size[1], offset[1])
                for k in generate_list_range(size[2], offset[2])
                for site in layer.sites
            ]
        else:
            # Confirm if the site_name matches with the registered one actually
